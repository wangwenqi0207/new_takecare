...
export default {
    ...
    getProvince(item){
      ...
      // console.log(item.regionId);
       this.$axios({
      url:'http://192.168.1.16:9005/insurance-intact-wechat-api/get_regions?parentId='+item.regionId,
      method: 'get'
      }).then(res=>{
        //console.log(res)
         this.cityL        = res.data;
   ...
   created() {
         var url="http://192.168.1.16:9005/insurance-intact-wechat-api/get_regions?parentId=0";
                    this.$axios({
                      method:'get',
                      url:url,
